...
def generate_tf_model():
    ...
    factory = BackendFactory.init("xla", target="cpu", optmax=False)

    # Initialize the model type
    ModelType = Model.init("tensorflow", backend_target="cpu")

    # Get the opset and exclude problematic operations
    opset = auto_opset(ModelType, factory)
    excluded_ops = ['Cholesky', 'Eigh']  # Add any other problematic ops here
...
def fuzz_model(model, max_iterations=1000):
    for i in range(max_iterations):
        ...
        try:
            outputs = model.run_eagerly(random_inputs)
...
def save_nan_scenario(model, inputs, outputs, directory="nan_scenario"):
...
def main():
    ...
    while True:
        print("Generating new TensorFlow model...")
